...
for i, t in enumerate([9,8,7,6]):
    axes[int(i/2), i % 2].set_xticks([-80,-60,-40,-20,0,20,40,60,80])
    axes[int(i/2), i % 2].contourf(yt,zt, MOC[t]/1e6,np.arange(-20,20.1,1),cmap='RdBu_r',corner_mask=False,extend='both')
    axes[int(i/2), i % 2].xaxis.set_major_formatter(mticker.FormatStrFormatter('$%i^{\circ}$'))
    axes[int(i/2), i % 2].yaxis.set_major_formatter(mticker.FormatStrFormatter('%i m'))
    axes[int(i/2), i % 2].xaxis.set_minor_locator(mticker.AutoMinorLocator())
