...
class GlobalFourDegreeSetup(VerosSetup):
    ...
    def set_coriolis(self, vs):
        vs.coriolis_t[...] = 2 * vs.omega * \
            np.sin(vs.yt[np.newaxis, :] / 180. * vs.pi)

    @veros_method(dist_safe=False, local_variables=[
        'kbot'
    ])
    def set_topography(self, vs):
        bathymetry_data = np.transpose(self._read_bath(vs, 'bathymetry')[12])
    ...
    def set_initial_conditions(self, vs):
        ...
        qnec_var = self._read_forcing(vs, 'q_nec')
        vs.qnec[2:-2, 2:-2,
                :] = np.array(qnec_var, dtype=str(qnec_var.dtype))
        ...
        if vs.enable_idemix:
            vs.forc_iw_bottom[2:-2, 2:-
                              2] = self._read_forcing(vs, 'tidal_energy') / vs.rho_0
            vs.forc_iw_surface[2:-2, 2:-
                               2] = self._read_forcing(vs, 'wind_energy') / vs.rho_0 * 0.2
